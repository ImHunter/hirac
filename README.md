...
В настройках appsettings.json может потребоваться указание каталога с библиотеками OneScript. За это отвечает элемент настройки lib.additional. В этой настройке необходимо указать путь к библиотекам. Обычно это [Каталог установки OneScript]\lib.

При указании путей в настройках *.json следует учитывать, что одиночный символ "\\" (обратный слеш) применяется, как служебный символ экранирования. Для корректного указания путей с использованием обратного слеша, необходимо удваивать такие слеши, либо использовать обычные слеши.

Может потребоваться настройка кодировки при чтении результатов rac. Это проявляется в нечитаемых реквизитах на кириллице - например, это нечитаемые имена пользователей. Настройка кодировки указывается в переменной окружения RAC_ENCODING (создать при необходимости). Варианты: UTF8, OEM, ANSI.
