...
Процедура ПроверитьОбновитьАвторизацию(ОбъектКластера)
	...
	Если НовыеПараметрыАвторизации = Неопределено Тогда
		НовыеПараметрыАвторизации = ПараметрыАвторизации;
	КонецЕсли;
...
Функция ПолныеОписанияОбъектовАгента(Знач ОписаниеАгента, Знач ТипОбъекта)
	...
	ТипЗнчОписаниеАгентаКластеры = ТипЗнч(ОписаниеАгента.Кластеры);
	Если ТипЗнчОписаниеАгентаКластеры = Тип("Массив") Тогда
		СписокКластеров = Новый ФиксированныйМассив(ОписаниеАгента.Кластеры);
	ИначеЕсли ТипЗнчОписаниеАгентаКластеры = Тип("ФиксированныйМассив") Тогда
		СписокКластеров = ОписаниеАгента.Кластеры;
	Иначе
		ВызватьИсключение("Непредусмотренный тип ОписаниеАгента.Кластеры: " + ТипЗнчОписаниеАгентаКластеры + ". Ожидается Массив, ФиксированныйМассив");
	КонецЕсли;
...
Процедура ДополнитьОписаниеОбъектаКластера(ПолноеОписаниеОбъекта, ТипОбъекта, Агент, Кластер)
	...
	// Видимо, используется более ранняя библиотека irac
	//МеткаАгента = СтрШаблон("%1:%2", Агент.АдресСервераАдминистрирования(), Агент.ПортСервераАдминистрирования());
	МеткаАгента = СтрШаблон("%1:%2", Агент.АдресRAS(), Агент.ПортRAS());
